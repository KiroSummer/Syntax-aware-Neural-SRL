...
def evaluate_tagger(model, batch_position_encoding, batched_dev_data, dev_dep_trees, evaluator, writer, global_step):
    predictions = []
    dev_loss = 0
    ...
    for i, batched_tensor in enumerate(batched_dev_data):
        ...
        word_inputs_seqs, predicate_inputs_seqs, syn_label_inputs_seqs, pes, answers, input_lengths, masks, padding_answers = \
            batch_data_variable(dev_dep_trees, batch_position_encoding, x, y, lengths, weights)

        if args.gpu:
            word_inputs_seqs, predicate_inputs_seqs, syn_label_inputs_seqs, input_lengths, masks, padding_answers = \
        ...
        output = model.forward(word_inputs_seqs, predicate_inputs_seqs, syn_label_inputs_seqs, pes, input_lengths)
        ...
        with Timer("Epoch%d" % epoch) as timer:
            ...
            if epoch % config.checkpoint_every_x_epochs == 0:
                with Timer('Evaluation'):
                    evaluate_tagger(model, batch_position_encoding, batched_dev_data, dev_dep_trees, evaluator, writer, global_step)
