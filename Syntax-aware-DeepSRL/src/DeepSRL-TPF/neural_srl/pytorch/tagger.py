...
class BiLSTMTaggerModel(nn.Module):
    ...
    def forward(self, x, x_predicate, x_tpf2, x_pes, x_lengths):
        embeddings, precidate_embeddings, tpf2_embeddings = \
            self.embedding(x), self.predicate_embedding(x_predicate), self.tpf2_embedding(x_tpf2)

        embeddings = torch.cat((embeddings, precidate_embeddings, tpf2_embeddings), 2)
