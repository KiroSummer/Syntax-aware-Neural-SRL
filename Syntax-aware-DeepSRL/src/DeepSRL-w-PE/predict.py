...
def get_scores(config, task, model_path, word_dict_path, label_dict_path, syntactic_dict_path, pattern_dict_path, input_path):
    ...
    with Timer('Running model'):
        ...
        for i, batched_tensor in enumerate(test_data):
            ...
            _, input_lengths, masks, padding_answers = \
                batch_data_variable(test_pattern, None, x, y, lengths, weights)

            if args.gpu:
                word_inputs_seqs, predicate_inputs_seqs, pattern_inputs_seqs, pattern_li_seqs, pattern_la_seqs, pattern_lp_seqs, \
                input_lengths, masks, padding_answers = \
                    word_inputs_seqs.cuda(), predicate_inputs_seqs.cuda(), pattern_inputs_seqs.cuda(), \
                    pattern_li_seqs.cuda(), pattern_la_seqs.cuda(), pattern_lp_seqs.cuda(), \
            ...
            sc = model.forward(word_inputs_seqs, predicate_inputs_seqs, pattern_inputs_seqs,
                               pattern_li_seqs, pattern_la_seqs, pattern_lp_seqs, input_lengths)
...
if __name__ == "__main__":
    ...
    for i in range(num_ensemble_models):
        ...
        syntactic_dict_path = os.path.join(args.model, 'syn_label_dict')
        pattern_dict_path = os.path.join(args.model, 'pattern_dict')
        print("model dict path: {}, word dict path: {}, label dict path: {}, syntactic dict path: {}, pattern dict path: {}".format( \
            model_path, word_dict_path, label_dict_path, syntactic_dict_path, pattern_dict_path))
