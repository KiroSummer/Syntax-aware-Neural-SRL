...
def evaluate_tagger(model, batch_position_encoding, batched_dev_data, dev_pattern, evaluator, writer, global_step):
    predictions = []
    dev_loss = 0
    ...
    for i, batched_tensor in enumerate(batched_dev_data):
        ...
        answers, input_lengths, masks, padding_answers = \
            batch_data_variable(dev_pattern, None, x, y, lengths, weights)
...
def train_tagger(args):
    ...
    with Timer("Pattern generating..."):
        ...
        train_pattern, data.pattern_dict = train_dep_trees.get_tpf2_dict(data.train_tensors)
        print("Extract {} training pattern features".format(len(train_pattern)))
        assert len(train_pattern) == len(data.train_tensors)
        data.pattern_dict.accept_new = False
        dev_pattern = dev_dep_trees.get_tpf2_dict(data.dev_tensors, data.pattern_dict)
        ...
        with Timer("Epoch%d" % epoch) as timer:
            ...
            for batched_tensor in train_data:  # for each batch in the training corpus
                ...
                word_inputs_seqs, predicate_inputs_seqs, pattern_inputs_seqs, pattern_li_seqs, pattern_la_seqs, pattern_lp_seqs, \
                    ...
                    batch_data_variable(train_pattern, None, x, y, lengths, weights)
            ...
            if epoch % config.checkpoint_every_x_epochs == 0:
                with Timer('Evaluation'):
                    evaluate_tagger(model, None, batched_dev_data, dev_pattern, evaluator, writer, global_step)
