...
class BiLSTMTaggerModel(nn.Module):
    ...
    def __init__(self, data, config, gpu_id=""):
        ...
        # self.tpf2_size = data.tpf2_dict.size()
        # self.tpf2_dim = int(config.tpf2_size)  # TPF2 size
        ...
        # self.tpf2_embedding = nn.Embedding(self.tpf2_size, self.tpf2_dim, padding_idx=0)
        ...
        # torch.nn.init.normal(self.tpf2_embedding.weight, 0.0, 0.01)
        torch.nn.init.normal(self.syntactic_label_embedding.weight, 0.0, 0.01)
        torch.nn.init.normal(self.pattern_embedding.weight, 0.0, 0.01)

        self.linear_projection = nn.Linear(4 * self.word_embedding_dim, self.word_embedding_dim, bias=False)
        torch.nn.init.orthogonal(self.linear_projection.weight)
        # Initialize BiLSTM
        self.bilstm = HighwayBiLSTM(
            input_size=3 * self.word_embedding_dim,  # word, predicate, tpf2
    ...
    def forward(self, x, x_predicate, x_pattern, x_li, x_la, x_lp, x_lengths):
        ...
        embeddings = torch.cat((embeddings, precidate_embeddings, pattern_representation), 2)
