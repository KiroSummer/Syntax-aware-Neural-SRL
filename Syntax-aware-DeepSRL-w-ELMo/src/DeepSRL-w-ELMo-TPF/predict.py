...
def get_scores(config, task, model_path, word_dict_path, label_dict_path, tpf_dict_path, input_path):
    ...
    with Timer('Running model'):
        ...
        for i, batched_tensor in enumerate(test_data):
            ...
            sc = model.forward(word_inputs_seqs, predicate_inputs_seqs, tpf_ids, elmo_representations, input_lengths)
...
if __name__ == "__main__":
    ...
    for i in range(num_ensemble_models):
        ...
        tpf_dict_path = os.path.join(args.model, 'tpf2_dict')
        print("model dict path: {}, word dict path: {}, label dict path: {}, tpf dict path: {}".format( \
            model_path, word_dict_path, label_dict_path, tpf_dict_path))
