...
def evaluate_tagger(model, batched_dev_data, dev_tpf2, dev_elmo_hdf5, evaluator, writer, global_step):
    predictions = []
    dev_loss = 0
    ...
    for i, batched_tensor in enumerate(batched_dev_data):
        ...
        word_inputs_seqs, predicate_inputs_seqs, tpf_ids, sentences_ids, answers, input_lengths, masks, padding_answers = \
            batch_data_variable(dev_tpf2, x, y, lengths, weights)
        ...
        output = model.forward(word_inputs_seqs, predicate_inputs_seqs, tpf_ids, elmo_representations, input_lengths)
        ...
        with Timer("Epoch%d" % epoch) as timer:
            ...
            if epoch % config.checkpoint_every_x_epochs == 0:
                with Timer('Evaluation'):
                    evaluate_tagger(model, batched_dev_data, dev_tpf2, dev_elmo_hdf5, evaluator, writer, global_step)
