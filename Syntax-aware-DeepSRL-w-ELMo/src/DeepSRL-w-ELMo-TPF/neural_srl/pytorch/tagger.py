...
class BiLSTMTaggerModel(nn.Module):
    ...
    def __init__(self, data, config, gpu_id=""):
        ...
        torch.nn.init.normal_(self.tpf2_embedding.weight, 0.0, 0.01)
    ...
    def forward(self, x, x_predicate, x_tpf_ids, elmo_representations, x_lengths):
        embeddings, precidate_embeddings, tpf_embeddings = \
            self.embedding(x), self.predicate_embedding(x_predicate), self.tpf2_embedding(x_tpf_ids)
        ...
        embeddings = torch.cat((embeddings, precidate_embeddings, tpf_embeddings, elmo_representations), 2)  # [B, L, H]
