...
class Biaffine(nn.Module):
    ...
    def reset_parameters(self):
        torch.nn.init.orthogonal(self.linear.weight)
...
class MyHighwayLSTMCell(nn.Module):
    ...
    def reset_parameters(self):
        ...
        nn.init.constant_(self.linear_ih.bias, 0.0)
    ...
    def forward(self, x, mask=None, hx=None, dropout=None):
        ...
        i, f, o, t, j = torch.sigmoid(i), torch.sigmoid(f + 1.0), torch.sigmoid(o), torch.sigmoid(t), torch.tanh(j)
        k = _x[:, self.hidden_size * 5:]

        c = f * _c + i * j
        c = mask * c + (1.0 - mask) * _c

        h = t * o * torch.tanh(c) + (1.0 - t) * k
