...
class BiLSTMTaggerModel(nn.Module):
    ...
    def __init__(self, data, config, gpu_id=""):
        ...
        self.tree_lstm = EncoderRNN(self.word_embedding_dim, self.word_embedding_dim)
        self.tree_lstm_projection = nn.Linear(2 * self.word_embedding_dim, self.word_embedding_dim, bias=False)
    ...
    def forward(self, x, x_predicate, x_syn_label_ids, x_pes, elmo_representations, x_lengths):
        embeddings, precidate_embeddings, syn_label_embeddings = \
            self.embedding(x), self.predicate_embedding(x_predicate), self.syntactic_label_embedding(x_syn_label_ids)
        ...
        syn_label_embeddings = syn_label_embeddings.transpose(0, 1)
        syn_label_embeddings, _ = self.tree_lstm(syn_label_embeddings, x_pes)
        syn_label_embeddings = syn_label_embeddings.transpose(0, 1)
        syn_label_embeddings = self.tree_lstm_projection(syn_label_embeddings)

        embeddings = torch.cat((embeddings, precidate_embeddings, syn_label_embeddings, elmo_representations), 2)  # [B, L, H]
