...
def get_scores(config, task, model_path, word_dict_path, label_dict_path, input_path):
    ...
    with Timer('Running model'):
        ...
        for i, batched_tensor in enumerate(test_data):
            ...
            word_inputs_seqs, predicate_inputs_seqs, sentences_ids, answers, input_lengths, masks, padding_answers = \
                batch_data_variable(x, y, lengths, weights)
            elmo_representations = test_elmo_hdf5.forward(sentences_ids, word_inputs_seqs.size()[-1],
                                                           [len(ans) for ans in answers])

            if args.gpu:
                word_inputs_seqs, predicate_inputs_seqs, input_lengths, masks, padding_answers = \
                    word_inputs_seqs.cuda(), predicate_inputs_seqs.cuda(), input_lengths.cuda(), masks.cuda(), padding_answers.cuda()
                elmo_representations = elmo_representations.cuda()

            sc = model.forward(word_inputs_seqs, predicate_inputs_seqs, elmo_representations, input_lengths)
