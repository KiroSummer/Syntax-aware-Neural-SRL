...
from neural_srl.pytorch.elmo import ELMO
...
def get_scores(config, task, model_path, word_dict_path, label_dict_path, syn_label_dict_path, pattern_dict_path, input_path):
    with Timer('Data loading'):
        ...
        syn_label_dict = Dictionary()
        pattern_dict = Dictionary()
        word_dict.load(word_dict_path)
        label_dict.load(label_dict_path)
        syn_label_dict.load(syn_label_dict_path)
        pattern_dict.load(pattern_dict_path)
        data = TaggerData(config, [], [], word_dict, label_dict, None, None)
        data.syntactic_dict = syn_label_dict
        data.pattern_dict = pattern_dict
    ...
    with Timer("Pattern generating..."):
        ...
        test_pattern = test_dep_trees.get_tpf2_dict(data.test_tensors, data.pattern_dict)
        print("Extract {} test Pattern features".format(len(test_pattern)))
        assert len(test_pattern) == len(data.test_tensors)
    ...
    with Timer('Running model'):
        ...
        for i, batched_tensor in enumerate(test_data):
            ...
            sentences_ids, answers, input_lengths, masks, padding_answers = \
                batch_data_variable(test_pattern, x, y, lengths, weights)
            elmo_representations = test_elmo_hdf5.forward(sentences_ids, word_inputs_seqs.size()[-1],
            ...
            sc = model.forward(word_inputs_seqs, predicate_inputs_seqs, pattern_inputs_seqs,
                                   pattern_li_seqs, pattern_la_seqs, pattern_lp_seqs, elmo_representations,
                                   input_lengths)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--input_elmo',
                        type=str,
                        default='',
                        required=True,
                        help='Path to the test elmo representation.')

    parser.add_argument('--input_dep_trees',
                        type=str,
                        default='',
                        required=True,
                        help='Path to the test dep trees.')
    ...
    for i in range(num_ensemble_models):
        ...
        syn_label_dict_path = os.path.join(args.model, 'syn_label_dict')
        pattern_dict_path = os.path.join(args.model, 'pattern_dict')
        print("model dict path: {}, word dict path: {}, label dict path: {}, syn_label_dict: {}, pattern dict path: {}".format( \
            model_path, word_dict_path, label_dict_path, syn_label_dict_path, pattern_dict_path))
