...
from ..TreeLSTM.Encoder import EncoderRNN
...
class BiLSTMTaggerModel(nn.Module):
    ...
    def __init__(self, data, config, gpu_id=""):
        ...
        self.syntactic_label_num = data.syntactic_dict.size()  # syntactic label size
        self.syntactic_label_dim = config.syn_label_size
        self.pattern_size = data.pattern_dict.size()
        self.pattern_dim = int(config.pattern_size)  # pattern size
        ...
        self.pattern_embedding = nn.Embedding(self.pattern_size, self.pattern_dim, padding_idx=0)

        self.embedding.weight.data.copy_(torch.from_numpy(np.asarray(data.embeddings[0])))
        torch.nn.init.normal_(self.predicate_embedding.weight, 0.0, 0.01)
        torch.nn.init.normal_(self.syntactic_label_embedding.weight, 0.0, 0.01)
        torch.nn.init.normal_(self.pattern_embedding.weight, 0.0, 0.01)

        self.linear_projection = nn.Linear(4 * self.word_embedding_dim, self.word_embedding_dim, bias=False)
        torch.nn.init.orthogonal_(self.linear_projection.weight)

        self.gamma = nn.Parameter(torch.FloatTensor([1.0]))
        self.softmax_weights = nn.ParameterList([nn.Parameter(torch.FloatTensor([0.0])) for _ in range(3)])
        # Initialize BiLSTM
        self.bilstm = HighwayBiLSTM(
            input_size=3 * self.word_embedding_dim + 1024,  # 1024
    ...
    def reset_parameters(self):
        ...
        nn.init.constant_(self.softmax.bias, 0.0)
    ...
    def forward(self, x, x_predicate, x_pattern, x_li, x_la, x_lp, elmo_representations, x_lengths):
        embeddings, precidate_embeddings, pattern_embeddings, li_embedding, la_embedding, lp_embedding = \
            self.embedding(x), self.predicate_embedding(x_predicate), self.pattern_embedding(x_pattern), \
            self.syntactic_label_embedding(x_li), self.syntactic_label_embedding(x_la), self.syntactic_label_embedding(x_lp)

        elmo_representations = elmo_representations.transpose(0, 1)
        ...
        elmo_representations = self.gamma * (normed_weights[0] * elmo_representations[0]
                + normed_weights[1] * elmo_representations[1] + normed_weights[2] * elmo_representations[2])

        pattern_representation = torch.cat((pattern_embeddings, li_embedding, la_embedding, lp_embedding), dim=2)
        pattern_representation = self.linear_projection(pattern_representation)

        embeddings = torch.cat((embeddings, precidate_embeddings, pattern_representation, elmo_representations), 2)  # [B, L, H]
