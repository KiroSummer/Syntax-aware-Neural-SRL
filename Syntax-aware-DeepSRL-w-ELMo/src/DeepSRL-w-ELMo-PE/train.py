...
def evaluate_tagger(model, batched_dev_data, dev_pattern, dev_elmo_hdf5, evaluator, writer, global_step):
    predictions = []
    dev_loss = 0
    ...
    for i, batched_tensor in enumerate(batched_dev_data):
        ...
        sentences_ids, answers, input_lengths, masks, padding_answers = \
            batch_data_variable(dev_pattern, x, y, lengths, weights)
        ...
        with Timer("Epoch%d" % epoch) as timer:
            ...
            if epoch % config.checkpoint_every_x_epochs == 0:
                with Timer('Evaluation'):
                    evaluate_tagger(model, batched_dev_data, dev_pattern, dev_elmo_hdf5, evaluator, writer, global_step)
